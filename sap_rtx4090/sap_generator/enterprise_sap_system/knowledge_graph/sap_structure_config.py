...
MASTER_SAP_SECTIONS = [
    ...
    SAPSection(
        ...
        subsections=[
            ...
            SAPSection("3.3", "Randomization", required=False,
                      ...
                      kb_tools=["get_stratification_specs", "get_stratification_balance_specs", "get_baseline_covariates"]),
    ...
    SAPSection(
        ...
        kb_tools=["get_statistical_method", "get_similar_trials"],
        subsections=[
    ...
    SAPSection(
        ...
        kb_tools=["get_population_definitions", "get_similar_trials"],
        subsections=[
    ...
    SAPSection(
        ...
        kb_tools=["get_demographics_baseline_specs", "get_baseline_covariates", "get_prior_therapy_specs", "get_performance_status_scales", "get_prognostic_scores"],
        subsections=[
            SAPSection("5A.1", "Demographics", required=True,
                      description="Age, sex, race, ethnicity, weight, height, BSA, BMI",
                      kb_tools=["get_demographics_baseline_specs", "get_baseline_covariates"]),
            SAPSection("5A.2", "Baseline Disease Characteristics", required=True,
                      description="ECOG PS, disease stage WITH SUBSTAGES (IIIA/IIIB/IIIC/IVA/IVB), time since diagnosis, measurable disease, disease-specific covariates. LIST ALL STAGE CATEGORIES.",
                      kb_tools=["get_demographics_baseline_specs", "get_baseline_covariates", "get_organ_function_specs", "get_performance_status_scales", "get_organ_function_scores"]),
            SAPSection("5A.3", "Prognostic Scores", required=False,
                      description="IPI, FLIPI, ISS, IMDC and other disease-specific scores",
                      kb_tools=["get_prognostic_scores", "get_baseline_covariates"]),
            SAPSection("5A.4", "Prior Anti-Cancer Therapy", required=True,
                      description="Number of prior lines, types (Chemotherapy, Targeted, Immunotherapy), specific agents, refractory status. INCLUDE TABLE SHELL for Prior Platinum-based Chemotherapy, Prior Radiotherapy, Prior CRT (concurrent chemoradiotherapy).",
                      kb_tools=["get_prior_therapy_specs", "get_baseline_covariates"]),
    ...
    SAPSection(
        ...
        kb_tools=["get_estimand_framework", "get_oncology_tfl_templates", "get_tumor_response_specs", "get_similar_trials"],
        subsections=[
            ...
            SAPSection("6.5", "Response Assessment Methodology", required=False,
                      condition="has_response_endpoint",
                      description="RECIST 1.1, Lugano, IWCLL, IMWG criteria with derivation tables",
                      kb_tools=["get_tumor_response_specs", "get_response_criteria", "get_all_response_criteria", "get_reference_sap_section"]),
    ...
    SAPSection(
        ...
        kb_tools=["get_statistical_method", "get_time_to_event_analysis", "get_similar_trials"],
        subsections=[
            ...
            SAPSection("7.5", "Multiplicity Adjustment", required=False,
                      ...
                      description="GSHf, alpha spending/recycling with specific boundaries",
                      kb_tools=["get_multiplicity_adjustment", "get_multiplicity_methods", "get_reference_sap_section"]),
            # Single-arm specific
    ...
    SAPSection(
        ...
        kb_tools=["get_censoring_rules", "get_reference_sap_section", "get_similar_trials"],
        subsections=[
    ...
    SAPSection(
        ...
        kb_tools=["get_missing_data_method", "get_data_handling_rules", "get_similar_trials"],
        subsections=[
    ...
    SAPSection(
        ...
        kb_tools=["get_sensitivity_analysis", "get_sensitivity_analysis_catalog", "get_reference_sap_section", "get_similar_trials"],
        subsections=[
    ...
    SAPSection(
        ...
        kb_tools=["get_subgroup_analysis_specs", "get_subgroup_specifications", "get_baseline_covariates", "get_reference_sap_section", "get_similar_trials"],
        subsections=[
            SAPSection("11.1", "Pre-specified Subgroups", required=True,
                      description="LIST ALL SUBGROUPS: Age (<65/>=65), Sex, Race, ECOG PS (0/1), Region, Smoking status, Disease stage, PD-L1 status, Histology, Prior therapy type, Response to prior CRT (CR/PR vs SD). Include ALL protocol-specified subgroups.",
                      kb_tools=["get_subgroup_specifications", "get_baseline_covariates"]),
    ...
    SAPSection(
        ...
        kb_tools=["get_safety_analysis_specs", "get_safety_tables", "get_safety_specifications", "get_ae_period_specifications", "get_similar_trials"],
        subsections=[
    ...
    SAPSection(
        ...
        kb_tools=["get_interim_analysis", "get_interim_analysis_specs", "get_reference_sap_section"],
        subsections=[
    ...
    SAPSection(
        ...
        kb_tools=["get_biomarker_endpoints", "get_baseline_covariates"],
        subsections=[
            ...
            SAPSection("14.3", "Biomarker Subgroup Analyses", required=False,
                      description="Subgroups by molecular markers (EGFR, ALK, PD-L1, BRCA, MSI, etc.)",
                      kb_tools=["get_biomarker_endpoints", "get_baseline_covariates"]),
        ]
    ),
    ...
    SAPSection(
        ...
        description="PRO instruments, scoring algorithms, MID thresholds, MMRM analysis",
        kb_tools=["get_pro_qol_analysis", "get_qol_analysis_specs", "get_reference_sap_section"],
        subsections=[
            SAPSection("15.1", "PRO Instruments", required=True,
                      description="LIST ALL SYMPTOMS BY INSTRUMENT: QLQ-C30 (Fatigue, Pain, Nausea/Vomiting, Dyspnea, Insomnia, Appetite Loss, Constipation, Diarrhea) + EQ-5D-5L (Mobility, Self-Care, Usual Activities, Pain/Discomfort, Anxiety/Depression) + For lung cancer: QLQ-LC13 (Dyspnoea, Coughing, Haemoptysis, Sore Mouth, Dysphagia, Peripheral Neuropathy, Alopecia, Chest Pain, Arm/Shoulder Pain). DO NOT just mention instrument names - LIST EACH SYMPTOM.",
                      kb_tools=["get_qol_analysis_specs", "get_pro_qol_analysis"]),
    ...
    SAPSection(
        ...
        subsections=[
            ...
            SAPSection("18.2", "Demographics and Baseline Tables", required=True,
                      description="All baseline covariate tables by disease type",
                      kb_tools=["get_demographics_baseline_specs", "get_baseline_covariates", "get_lymphoma_tables"]),
            SAPSection("18.3", "Efficacy Tables", required=True,
                      description="BOR table MUST include: CR, PR, SD, PD, NE, ORR (CR+PR), DCR (CR+PR+SD). Include all response categories with 95% CI.",
            ...
            SAPSection("18.5", "Figures", required=True,
                      description="MUST include: Kaplan-Meier plots (PFS, OS), Forest plot (subgroups), Waterfall plot (best % change in tumor size), Swimmer plot (duration of response).",
    ...
    SAPSection(
        ...
        kb_tools=["get_follow_up_analysis_specs"],
        subsections=[
    ...
    SAPSection(
        ...
        subsections=[
            ...
            SAPSection("A.2", "Time-to-Event Derivation Rules", required=False,
                      condition="has_tte_endpoints",
                      description="Circumstance tables for each TTE endpoint (DOR, PFS, OS) - TABLE A.2.1 (PFS), TABLE A.2.2 (OS)",
                      kb_tools=["get_tte_derivation_tables", "get_censoring_rules"]),
