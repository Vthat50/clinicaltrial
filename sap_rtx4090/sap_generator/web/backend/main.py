...
@app.post("/generate-tlf-shells/{job_id}")
async def generate_tlf_shells(
    job_id: str,
    ...
    try:
        ...
        # Call the real rules-engine-based generator
        ...
        if format == "docx":
            ...
            filename = f"TLF_Shells_{protocol_id}.docx"
            ...
            # Markdown format — return TLFShellResponse for backward compat
            return TLFShellResponse(
                success=True,
                message="Generated TLF shells via rules engine",
                tables=[],
                listings=[],
                figures=[],
                total_outputs=0,
...
async def generate_tlf_shells_direct(
    ...
    This is the "Quick Protocol" mode — no SAP generation required.
    Accepts a protocol PDF/DOCX/TXT, extracts facts via Claude,
    and produces TLF shells in one step.
    ...
    try:
        ...
        if format == "docx":
            ...
            if not output:
                raise HTTPException(status_code=500, detail="Failed to generate .docx")
            docx_filename = f"TLF_Shells_{protocol_id}.docx"
            return StreamingResponse(
                BytesIO(output),
                media_type="application/vnd.openxmlformats-officedocument.wordprocessingml.document",
        ...
        logger.error(f"[TLF Direct] Generation failed: {e}")
        if format == "docx":
            raise HTTPException(status_code=500, detail=f"TLF generation failed: {str(e)}")
        return TLFShellResponse(
            success=False,
